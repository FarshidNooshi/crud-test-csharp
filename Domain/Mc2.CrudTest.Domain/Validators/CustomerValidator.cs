...
public class CustomerValidator : AbstractValidator<Customer>
{
    ...
    public CustomerValidator(ICustomerRepository repository)
    {
        ...
        RuleFor(x => x.BankAccountNumber).Must(BeAValidBankAccountNumber)
            .WithMessage("A valid bank account number is required.");
    ...
    private bool BeAValidMobileNumber(Customer costumer, string phoneNumber, ValidationContext<Customer> context)
    {
        ...
        try
        {
            ...
            if (numberProto.NumberType != PhoneNumberUtil.PhoneNumberType.MOBILE)
            {
